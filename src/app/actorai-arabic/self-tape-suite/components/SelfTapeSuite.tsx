  ...
  const generateAutoNotes = (duration: number): TakeNote[] => {
    ...
    for (let i = 0; i < numNotes; i++) {
      ...
      const content = isPositive
        ? (positiveComments[Math.floor(Math.random() * positiveComments.length)] ?? "أداء جيد")
        : (neutralComments[Math.floor(Math.random() * neutralComments.length)] ?? "ملاحظة");
      notes.push({
        id: `auto-${Date.now()}-${i}`,
